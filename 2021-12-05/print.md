...
身為前端人員當然有時也會被要求幫忙處理列印的相關事情 , 當我開始處理後 , 發現這個 issue 其實不太容易處理

因此在此紀錄 , 免得自己的金魚腦 , 1 個月後就全部忘記 `當初` 到底是如何處理的

## 釐清目標

需求是要印出下方的檔案 (由於原本的檔案內容有公司機密 , 因此我拿自己的台新銀行 `個人支付紀錄` 做為替代)

> 台新銀行匯出的 PDF

先列出 PM 收集到的需求 - 點擊 `列印` 按鈕後 , 可以直接列印 `當季損益表`

接到這個需求時 , 我初步將問題大概切分成 2 塊 , 來思考 "前端要處理甚麼 ?"
...
第一步當然是做出一個 A4 大小的頁面 , 讓 html2canvas 可以進行 printScreen 的行為 
